...
namespace MessagingPlatformAPI.Services.Implementation
{
    public class UserConnectionService : IUserConnectionService
    {
        ...
        public async Task<SimpleResponseDTO> Delete(string connectionId)
        {
            var userConn = await _base.Get(c => c.ConnectionId == connectionId);
            if (userConn == null) return new SimpleResponseDTO() { IsSuccess = false, Message = "Chat is not found" };
            await _base.Remove(userConn);
            return new SimpleResponseDTO() { IsSuccess = true, Message = "user Connection deletion is done" };
        }
    }
}
